...
const LoadMore = ({ sort, filter, setFilter }) => {
  ...
  const filterHandler = () => {
    if (filter === 0) {
      ...
      http
        .get(`/categories/${filter}/products?offset=0&limit=${count}`)
        .then((response) => {
          dispatch({ type: "payload", data: response.data });
          dispatch({ type: "sort", value: sort });
          setFilter("update");
  ...
  return (
    <button
      ...
      className="bg-[#da7d26] mx-auto mt-7 rounded-lg px-3 py-1 grid items-center justify-center text-white font-semibold"
    >
      {status === "loaded" ? "Load More!" : loadingHandler("Loading")}
    </button>
  );
};
